...
const size        = 100;          // Grid size (units)
const divisions   = 100;          // Number of grid divisions
const scaleEffect = 0.2;          // Wave amplitude multiplier
const cameraPos   = [0, -100, 4]; // Initial camera position [x, y, z]
const particlesVolume = 2;        // Particle spread multiplier
...
function getWaveHeight(x, y, time, distanceFromCamera) {
  ...
  const intensityMultiplier = Math.exp(1 * distanceFromCamera) - 4.6;
...
function AnimatedGrid() {
  ...
  // Create node positions (every 2nd vertex to reduce density)
...
function Particles() {
  ...
  const particles = useMemo(() => {
	...
	for (let i = 0; i < particleCount; i++) {
	  ...
	  positions[i * 3 + 2] = particlesVolume * (Math.random() - 0.5) * 10; // Z
...
/**
 * Main scene setup with camera, lights, and controls
 */
function Scene() {
 ...
 * Creates the Assassin's Creed-style animated grid loading screen
 */
export default function AssassinsCreedGrid() {
  return (
	<Scene />
  );
}
