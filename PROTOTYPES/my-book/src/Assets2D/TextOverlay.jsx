...
const TextOverlay = ({ leftContent, rightContent, open, onClose }) => {
    ...
    const extractText = (element) => {
        ...
        if (element.type === 'h1' || element.type === 'h2' || element.type === 'h3' || element.type === 'h4') {
            ...
            items.forEach(item => {
                if (item?.type === 'li') {
                    text += `• ${extractChildren(item.props.children)}\n`
                }
            })
            text += '\n'
            ...
            text += extractChildren(children).toUpperCase()
    ...
    const extractAllText = (content) => {
        ...
        if (content?.props?.sx?.background) {
            return extractChildren(content.props.children)
        }
                ...
                >
                    ...
                    {nothingToDisplay ? (
                        <Box sx={{ width: '100%', textAlign: 'center', py: 8 }}>
                            <Typography variant="h4" sx={{ color: '#fff', fontWeight: 'bold', textShadow: '2px 2px 4px rgba(0,0,0,0.8)' }}>
                                Nothing to display!
                            </Typography>
                        </Box>
                    ) : (
                                ...
                                <Box sx={{ flex: 1, minWidth: 0 }}>
                                    <Box sx={{ display: 'flex', alignItems: 'center', mb: 3, gap: 2 }}>
                                        <Tooltip title={speaking ? "Stop Reading" : "Read Aloud"}>
                                            <IconButton
                                                onClick={() => speaking ? handleStopSpeech() : handleSpeak(leftText)}
                                                sx={{
                                                    bgcolor: speaking ? '#f44336' : '#2196f3',
                                                    color: 'white',
                                                    '&:hover': { 
                                                        bgcolor: speaking ? '#d32f2f' : '#1976d2' 
                                                    },
                                                }}
                                            >
                                                <VolumeUpIcon />
                            ...
                            {rightText && (
                                <Box sx={{ flex: 1, minWidth: 0 }}>
                                    <Box sx={{ display: 'flex', alignItems: 'center', mb: 3, gap: 2 }}>
                                        <Tooltip title={speaking ? "Stop Reading" : "Read Aloud"}>
                                            <IconButton
                                                onClick={() => speaking ? handleStopSpeech() : handleSpeak(rightText)}
                                                sx={{
                                                    bgcolor: speaking ? '#f44336' : '#2196f3',
                                                    color: 'white',
                                                    '&:hover': { 
